...
class HashTagSearchExample(HashTagSearch):
    ...
    def save_results(self, instagram_results):
        ...
        for i, post in enumerate(instagram_results):
            ...
            #print(post.processed_text())
            my_hashtag_list.extend(post.processed_text().split())

            if self.total_posts>200:
                print("Hashtags:")
                for i in my_hashtag_list:
                    if(str(i).startswith('#')):
                        my_hash_list.append(i)
                freq = {} 
                for item in my_hash_list: 
                    if (item in freq): 
                        freq[item] += 1
                    else: 
                        freq[item] = 1
              
                file = open("Hashtags.text","w+")
                for i in range(100):
                    maximum = max(freq.values())
                    print(maximum,list(freq.keys())[list(freq.values()).index(maximum)])
                    file.write(list(freq.keys())[list(freq.values()).index(maximum)]+"\n")
                    freq.pop(list(freq.keys())[list(freq.values()).index(maximum)])
                file.close()
                sys.exit(1)

if __name__ == '__main__':
    log.basicConfig(level=log.INFO)
    topic = input("Enter the topic of the Hashtags:")
    HashTagSearchExample().extract_recent_tag(topic)